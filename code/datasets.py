...
#              transform=None, preprocess=None):
...
#             else:
#                 re_img = img
#             # print(preprocess(re_img).size())
#             ret.append(preprocess(re_img))
#
#     return ret
...
class TextDataset(data.Dataset):
    def __init__(self, data_dir, split='train',
        ...
        #     _, self.preprocess = clip.load("RN101", device=self.device)
        self.imsize = []
        for i in range(cfg.TREE.BRANCH_NUM):
        ...
        self.filenames, self.captions, self.cap_lens, self.n_words = self.load_text_data(data_dir, split)
        self.ixtoword = _Tokenizer()
    ...
    def load_text_data(self, data_dir, split):
        ...
        if not os.path.isfile(filepath):
            train_captions, train_lens = self.load_captions(data_dir, train_names)
            test_captions, test_lens = self.load_captions(data_dir, test_names)
            print(len(train_lens))
            train_captions = clip.tokenize(train_captions)
            print(train_captions.size())
            test_captions = clip.tokenize(test_captions)
            print(test_captions.size())
            # raise KeyError
            # train_captions, test_captions, ixtoword, wordtoix, n_words = \
            #     self.build_dictionary(train_captions, test_captions)
            with open(filepath, 'wb') as f:
                pickle.dump([train_captions, test_captions, train_lens, test_lens], f, protocol=2)
        ...
        if split == 'train':
            ...
            cap_lens = test_lens
        return filenames, captions, cap_lens, n_words
