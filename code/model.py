...
class NEXT_STAGE_G(nn.Module):
    ...
    def forward(self, h_code, c_code, cond_embs, mask, cap_lens):
        """
        ...
        """
        # Memory Writing
        word_embs_T = torch.transpose(cond_embs, 1, 2).contiguous()
        ...
        gate2 = self.B(h_code_avg_T).repeat(1, 1, cond_embs.size(2))
        writing_gate = torch.sigmoid(gate1 + gate2)
        h_code_avg = h_code_avg.repeat(1, 1, cond_embs.size(2))
        memory = self.M_w(cond_embs) * writing_gate + self.M_r(h_code_avg) * (1 - writing_gate)
...
class G_NET(nn.Module):
    ...
    def forward(self, z_code, sent_emb, word_embs, mask, cap_lens):
        ...
        if cfg.TREE.BRANCH_NUM > 1:
            s_embs = sent_emb.repeat(word_embs.size(2), 1, 1)
            s_embs = s_embs.permute(1, 2, 0)
            h_code2, att1 = self.h_net2(h_code1, c_code, s_embs, mask, cap_lens)
