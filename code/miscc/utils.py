...
def drawCaption_ITM(convas, captions, cap_lens, ixtoword, vis_size, off1=2, off2=2):
    num = captions.size(0)
    img_txt = Image.fromarray(convas)
    # get a font
    # fnt = None  # ImageFont.truetype('Pillow/Tests/fonts/FreeMono.ttf', 50)
    ...
    for i in range(num):
        ...
        lens = cap_lens[i]
        sentence = ''
        for j, w in enumerate(cap[1:lens + 1]):
            if w == 0:
                break
            word = ixtoword.decoder[w].replace('</w>', ' ')
            d.text(((j + off1) * (vis_size + off2), i * FONT_MAX), '%d:%s' % (j, word[:6]),
                   font=fnt, fill=(255, 255, 255, 255))
            sentence += word
        sentence_list.append(sentence)
...
def build_super_images_ITM(real_imgs, captions, ixtoword, cap_lens,
                            attn_maps, att_sze, lr_imgs=None,
                            batch_size=cfg.TRAIN.BATCH_SIZE,
                            max_word_num=cfg.TEXT.WORDS_NUM):
    nvis = 8
    ...
    # real_imgs = (real_imgs[:] + 2) / 4.0 * 255.0
    # print(real_imgs)
    real_imgs = \
    ...
    # [-1, 1] --> [0, 1]
    # [-2, 2] --> [0, 1]
    real_imgs.add_(2).div_(4).mul_(255)
    real_imgs = real_imgs.data.numpy()
    # b x c x h x w --> b x h x w x c
    real_imgs = np.transpose(real_imgs, (0, 2, 3, 1))
    pad_sze = real_imgs.shape
    ...
    text_map, sentences = \
        drawCaption_ITM(text_convas, captions, cap_lens, ixtoword, vis_size)
    text_map = np.asarray(text_map).astype(np.uint8)
