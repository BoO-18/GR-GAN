...
from GR_trainer import condGANTrainer as trainer
...
if __name__ == "__main__":
    ...
    algo = trainer(output_dir, dataloader, dataset.n_words, dataset.ixtoword, resume, args, dataset)

    start_t = time.time()
    if cfg.TRAIN.FLAG:

        seed_dir = output_dir + '/seed.txt'
        f = open(seed_dir, 'a')
        f.writelines(['seed:', str(args.manualSeed)])
        f.close()
        copyfile(sys.argv[0], output_dir + "/" + sys.argv[0])
        copyfile("GR_trainer.py", output_dir + "/" + "GR_trainer.py")
        ...
        if cfg.B_VALIDATION:
            algo.sampling(split_dir, num_samples=30000)  # generate images for the whole valid dataset
        else:
